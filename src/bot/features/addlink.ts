...
// 🚀 /addlink <Telegram链接> [描述]
feature.command('addlink', logHandle('command-addlink'), async (ctx) => {
  const args = ctx.message?.text?.split(' ').slice(1);
  if (!args || args.length === 0) {
    return ctx.reply(
      '⚠️ 用法：`/addlink <Telegram链接> [描述]`\n\n例如：\n`/addlink https://t.me/mygroup 这是一个学习群`',
      { parse_mode: 'Markdown' }
    );
  }
  ...
  const user = ctx.from!;
  const exists = await LinkModel.findOne({ url: link });
  if (exists) return ctx.reply('⚠️ 该链接已被收录。');
  ...
  await LinkModel.create({
    ...
    url: link,
    type: normalizedType, // ✅ 归一化后
    chat_type_raw: chatMeta.type || null, // 可选保存原始类型
...
export { composer as addLinkFeature };
